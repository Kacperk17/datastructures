...
public class TreeMap<K, V> extends AbstractSortedMap<K, V> {
    ...
    protected static class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
        ...
        private void relink(Node<Entry<K, V>> parent, Node<Entry<K, V>> child, boolean makeLeftChild) {
            ...
            child.setParent(parent); // set parent node of given child node to given parent

            // set left or right based on input
            if(makeLeftChild) {
        ...
        public void rotate(Position<Entry<K, V>> p) {
            ...
            Node<Entry<K, V>> givenNode = (Node<Entry<K, V>>) p;

            Node<Entry<K, V>> y = givenNode.getParent(); // parent node
            Node<Entry<K, V>> z = y.getParent(); // grandparent node (might not exist)

            if(z == null) { // given node has no grandparent node

                // set given node as root
                root = givenNode;
                givenNode.setParent(null);

            } else {
                relink(z, givenNode, y == z.getLeft()); //given node becomes child of z from same position as y
            }

            if(givenNode == y.getLeft()) { // if given node is still left child of y

                // given node's right child is now y's left child and y is now given node's left child.
                relink(y, givenNode.getRight(), true);
                relink(givenNode, y, false);
            } else {
                relink(y, givenNode.getLeft(), false);
                relink(givenNode, y, true);
            }
        ...
        public Position<Entry<K, V>> restructure(Position<Entry<K, V>> x) {
            ...
            if((x == right(y) && y == right(z)) || (x == left(y) && y == left(y))) {
                rotate(y);
                return y;
            } else {
                rotate(x);
    ...
    private Position<Entry<K, V>> treeSearch(Position<Entry<K, V>> p, K key) {
        ...
        if(isExternal(p)) { // if position is external
            return p; // unsuccessful
        }
    ...
    protected Position<Entry<K, V>> treeMin(Position<Entry<K, V>> p) {
        ...
        Position<Entry<K, V>> newP = p;

        while(isInternal(newP)) {
            newP = left(newP);
        }
        return parent(newP);
    ...
    public V put(K key, V value) throws IllegalArgumentException {
        ...
        Position<Entry<K, V>> positionOfValue = treeSearch(root(), key);


        if(isExternal(positionOfValue)) {
            expandExternal(positionOfValue, newEntry);
            return null;
        } else {
            V valueToReplace = positionOfValue.getElement().getValue();
            set(positionOfValue, newEntry);
    ...
    public V remove(K key) throws IllegalArgumentException {
        ...
        if(isExternal(p)) { // key not found
            ...
            V valueToDelete = p.getElement().getValue();
    ...
    public String toString() {
        return tree.toString().replace("⦰, ", "").replace(", ⦰", "");
    }
    ...
    public Iterable<Entry<K, V>> subMap(K fromKey, K toKey) throws IllegalArgumentException {
        ...
        if(compare(fromKey, toKey) < 0) {
            subMapHelper(fromKey, toKey, root(), buffer);
        }

        return buffer;
    }

    // recursive helper function for submap method
    private void subMapHelper(K fromKey, K toKey, Position<Entry<K, V>> p, ArrayList<Entry<K, V>> buffer) {


        if(isInternal(p)) {

            if(compare(p.getElement(), fromKey) < 0) { // p is less than fromKey therefore we check right subtree
                subMapHelper(fromKey, toKey, right(p), buffer);
            }
            else {
                subMapHelper(fromKey, toKey, left(p), buffer);
