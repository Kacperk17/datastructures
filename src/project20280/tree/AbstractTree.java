...
public abstract class AbstractTree<E> implements Tree<E> {
    ...
    private void preorderSubtree(Position<E> p, List<Position<E>> snapshot) {
        ...
        snapshot.add(p); // add root to first element in snapshot

        // for every child in p
        for(Position<E> child : children(p)) {
    ...
    public Iterable<Position<E>> preorder() {
        ...
        if(!isEmpty()) {
            preorderSubtree(root(), snapshot); // fill snapshot
        }
    ...
    private void postorderSubtree(Position<E> p, List<Position<E>> snapshot) {
        ...
        // add p to snapshot after recursively traversing subtrees
        snapshot.add(p);
    ...
    public Iterable<Position<E>> breadthfirst() {
        ...
        if(isEmpty()) {
            ...
            while(!queue.isEmpty()) {
                ...
                // dequeue position from queue then add it to the snapshot
                Position<E> p = queue.dequeue();
                snapshot.add(p);

                // enqueue each child to the queue
                for(Position<E> child : children(p)) {
                    queue.enqueue(child);
                }
